...
// DATE:   04/27/2022
//
// A simple server program that utilizes Unix sockets to receive infromation from clients.
// The the client can use the commands get, clear or add to change or receive the value of an accumulator "count"
...
void* do_work(void *generic_ptr)
{
  ...
  else if( recvln[0] == 'c' && recvln[1] == 'l' && recvln[2] == 'e' && recvln[3] == 'a' && recvln[4] == 'r' && recvln[5] == '\n' ) //if we read the input clear
  {
  	count = 0;//reset the accumulator
  }
  else if(recvln[0] == 'a' && recvln[1] == 'd' && recvln[2] == 'd' && recvln[3] == ' ')//if we read add <integer>
  {
  	
  	int add = atoi(&recvln[4]);//extract that integer from the whole array
  	count += add;//add it to the accumulator
  	
  	
  	//send this new accumulator back to the client
  ...
  else if(recvln[0] == 'g' && recvln[1] == 'e' && recvln[2] == 't' && recvln[3] == '\n' )//if we read get
  {
  	//simply send the actual value of the accumulator to the client
  	string buffer;
	buffer = to_string(count);
	buffer+='\n';
